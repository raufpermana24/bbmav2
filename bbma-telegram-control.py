...
║  ANALISIS TAMBAHAN:                                              ║
║  ZONE ZERO LOSS (ZZL):                                          ║
║  • Kondisi di mana SEMUA candle (OHLC) dan semua indikator MA   ║
║    (MALO5, MALO10, MAHI5, MAHI10) berada di SATU SISI MidBB    ║
║  • Di ATAS MidBB → ZZL BUY | Di BAWAH MidBB → ZZL SELL        ║
║  • Syarat tambahan: EMA50 harus di sisi BERLAWANAN dari MidBB   ║
║    (EMA50 < MidBB untuk ZZL BUY, EMA50 > MidBB untuk ZZL SELL) ║
║  • Ditampilkan otomatis di setiap notifikasi sinyal Telegram    ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
def compute_signals(df: pd.DataFrame) -> dict:
    ...
    # Hitung Zone Zero Loss sekali untuk semua sinyal pada candle ini
    zzl_result = check_zone_zero_loss(df, idx=0)
    for k in signals:
...
def compute_signals_at(df: pd.DataFrame, idx: int) -> dict:
    ...
    # Hitung Zone Zero Loss untuk candle pada idx ini
    zzl_result = check_zone_zero_loss(df, idx=idx)
    for k in signals:
        signals[k]['price'] = float(c['close'])
        signals[k]['time']  = str(c['timestamp'])
        signals[k]['zzl']   = zzl_result
...
# ==========================================
# 11b. ZONE ZERO LOSS ANALYSIS
# ==========================================
def check_zone_zero_loss(df: pd.DataFrame, idx: int = 0) -> dict:
    """
    Analisis Zone Zero Loss (ZZL):
    Kondisi ideal di mana SEMUA candle dan indikator MA berada di SATU SISI
    dari Mid BB (di atas untuk BUY, di bawah untuk SELL), dengan syarat
    TAMBAHAN: MA50 (EMA50) harus berada di SISI BERLAWANAN dari Mid BB.

    Syarat ZZL BUY:
      - close   > midBB
      - open    > midBB
      - high    > midBB
      - low     > midBB   ← seluruh body & shadow candle di atas Mid BB
      - malo5   > midBB
      - malo10  > midBB
      - mahi5   > midBB
      - mahi10  > midBB
      - ema50   < midBB   ← MA50 di sisi BERLAWANAN (bawah Mid BB)

    Syarat ZZL SELL:
      - close   < midBB
      - open    < midBB
      - high    < midBB   ← seluruh body & shadow candle di bawah Mid BB
      - low     < midBB
      - malo5   < midBB
      - malo10  < midBB
      - mahi5   < midBB
      - mahi10  < midBB
      - ema50   > midBB   ← MA50 di sisi BERLAWANAN (atas Mid BB)

    Parameters:
        df  : DataFrame dengan indikator sudah dihitung (add_indicators)
        idx : 0 = candle closed terbaru (iloc[-2]), 1 = sebelumnya, dst.

    Returns dict dengan keys:
        'zzl'        : 'BUY' | 'SELL' | None
        'candle_ok'  : bool — seluruh candle (OHLC) di satu sisi midBB
        'ma_ok'      : bool — semua MA di satu sisi midBB
        'ma50_ok'    : bool — MA50 di sisi berlawanan
        'details'    : str — keterangan ringkas
    """
    result = {
        'zzl':       None,
        'candle_ok': False,
    ...
    if df is None or len(df) < idx + 4:
        result['details'] = 'Data tidak cukup'
        return result

    c = df.iloc[-(idx + 2)]

    required = ['open', 'high', 'low', 'close',
                'midBB', 'malo5', 'malo10', 'mahi5', 'mahi10', 'ema50']
    for col in required:
        if pd.isna(c.get(col)):
            result['details'] = f'Kolom {col} NaN'
            return result

    mid   = c['midBB']
    o, h, l, cl = c['open'], c['high'], c['low'], c['close']
    ma5l, ma10l  = c['malo5'],  c['malo10']
    ma5h, ma10h  = c['mahi5'],  c['mahi10']
    ema50        = c['ema50']

    # ── Cek BUY ──────────────────────────────────────────────────
    candle_buy = (o > mid and h > mid and l > mid and cl > mid)
    ma_buy     = (ma5l > mid and ma10l > mid and ma5h > mid and ma10h > mid)
    ma50_sell_side = (ema50 < mid)   # MA50 di sisi berlawanan untuk ZZL BUY

    if candle_buy and ma_buy and ma50_sell_side:
        result['zzl']       = 'BUY'
        result['candle_ok'] = True
        result['ma_ok']     = True
        result['ma50_ok']   = True
        result['details']   = (
            f"✅ ZZL BUY — Semua candle & MA di ATAS MidBB "
            f"| EMA50 {ema50:.6g} < MidBB {mid:.6g} ✅"
        )
        return result

    # ── Cek SELL ─────────────────────────────────────────────────
    candle_sell = (o < mid and h < mid and l < mid and cl < mid)
    ma_sell     = (ma5l < mid and ma10l < mid and ma5h < mid and ma10h < mid)
    ma50_buy_side = (ema50 > mid)    # MA50 di sisi berlawanan untuk ZZL SELL

    if candle_sell and ma_sell and ma50_buy_side:
        result['zzl']       = 'SELL'
        result['candle_ok'] = True
        result['ma_ok']     = True
        result['ma50_ok']   = True
        result['details']   = (
            f"✅ ZZL SELL — Semua candle & MA di BAWAH MidBB "
            f"| EMA50 {ema50:.6g} > MidBB {mid:.6g} ✅"
        )
        return result

    # ── Tidak memenuhi syarat ZZL — beri penjelasan parsial ──────
    parts = []
    ...
    if cl > mid:
        c_ok_buy  = candle_buy
        m_ok_buy  = ma_buy
        m50_ok_buy= ma50_sell_side
        if not c_ok_buy:
            fail = [x for x, v in [('open', o > mid), ('high', h > mid),
                                    ('low', l > mid)] if not v]
            parts.append(f"Candle BUY gagal: {', '.join(fail)} menyentuh bawah MidBB")
        if not m_ok_buy:
            fail = [nm for nm, v in [('MALO5', ma5l > mid), ('MALO10', ma10l > mid),
                                     ('MAHI5', ma5h > mid), ('MAHI10', ma10h > mid)] if not v]
            parts.append(f"MA BUY gagal: {', '.join(fail)} di bawah MidBB")
        if not m50_ok_buy:
            parts.append(f"MA50 gagal: EMA50 {ema50:.6g} harus < MidBB {mid:.6g}")
    else:
        c_ok_sell  = candle_sell
        m_ok_sell  = ma_sell
        m50_ok_sell= ma50_buy_side
        if not c_ok_sell:
            fail = [x for x, v in [('open', o < mid), ('high', h < mid),
                                    ('low', l < mid)] if not v]
            parts.append(f"Candle SELL gagal: {', '.join(fail)} menyentuh atas MidBB")
        if not m_ok_sell:
            fail = [nm for nm, v in [('MALO5', ma5l < mid), ('MALO10', ma10l < mid),
                                     ('MAHI5', ma5h < mid), ('MAHI10', ma10h < mid)] if not v]
            parts.append(f"MA SELL gagal: {', '.join(fail)} di atas MidBB")
        if not m50_ok_sell:
            parts.append(f"MA50 gagal: EMA50 {ema50:.6g} harus > MidBB {mid:.6g}")

    result['details'] = ' | '.join(parts) if parts else 'Tidak ada ZZL'
    return result


def _format_zzl_block(zzl: dict) -> str:
    """Format ringkasan ZZL untuk notifikasi Telegram."""
    z = zzl.get('zzl')
    if z == 'BUY':
        return (
            f"🏆 <b>ZONE ZERO LOSS : ✅ AKTIF (BUY)</b>\n"
            f"  • Seluruh candle (OHLC) di ATAS MidBB ✅\n"
            f"  • Semua MA (5H/10H/5L/10L) di ATAS MidBB ✅\n"
            f"  • EMA50 di BAWAH MidBB (sisi berlawanan) ✅"
        )
    elif z == 'SELL':
        return (
            f"🏆 <b>ZONE ZERO LOSS : ✅ AKTIF (SELL)</b>\n"
            f"  • Seluruh candle (OHLC) di BAWAH MidBB ✅\n"
            f"  • Semua MA (5H/10H/5L/10L) di BAWAH MidBB ✅\n"
            f"  • EMA50 di ATAS MidBB (sisi berlawanan) ✅"
        )
    else:
        detail = zzl.get('details', 'Belum terpenuhi')
        return (
            f"⚪ <b>ZONE ZERO LOSS : ❌ BELUM AKTIF</b>\n"
            f"  <i>{detail}</i>"
        )
...
def send_telegram_alert(symbol: str, signal_name: str, timeframe: str,
    ...
    # Blok Zone Zero Loss
    zzl_blk = _format_zzl_block(data.get('zzl', {}))
        ...
        for tf in TIMEFRAMES:
            ...
            for idx in range(max_back - 1, -1, -1):
                ...
                for sig_name, sig_data in sigs.items():
                    ...
                    zzl_blk     = _format_zzl_block(sig_data.get('zzl', {}))
