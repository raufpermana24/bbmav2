...
# SETTING STRATEGI (TOP LOSER 5 MENIT)
TIMEFRAME = '5m'        # Scalping Cepat
VOL_MULTIPLIER = 2.0    # Volume minimal 2x rata-rata 
LIMIT = 400             # Data candle
TOP_COIN_COUNT = 100    # Fokus ke 100 Koin Penurunan Terbesar (Top Losers)
MAX_THREADS = 15        

OUTPUT_FOLDER = 'loser_5m_results'
...
def send_telegram_alert(symbol, data, image_path):
    ...
    # Kategorisasi NATR untuk info cepat
    natr_val = data['natr']
    ...
    caption = (
        f"📉 <b>TOP LOSER REVERSAL (5M)</b>\n"
        f"💎 <b>{symbol}</b>\n"
        f"🔻 <b>24h Drop:</b> {data['24h_change']:.2f}%\n"
        f"──────────────────────\n"
...
def generate_chart(df, symbol, signal_info):
    try:
        ...
        # Tambahkan info NATR di judul chart
...
# 5. DATA ENGINE
# ==========================================
def get_loser_symbols(limit=100):
    try:
        ...
        sorted_tickers = sorted(valid_tickers, key=lambda x: x['percentage'] if x['percentage'] else 999, reverse=False)
...
def analyze_chart_pattern(df):
    ...
    if len(troughs) >= 2:
        if abs(troughs.iloc[-1]['low'] - troughs.iloc[-2]['low']) / troughs.iloc[-2]['low'] < 0.003:
            if (troughs.iloc[-1].name - troughs.iloc[-2].name) > 5:
                pattern_found.append("Double Bottom (Reversal)")
    ...
    if slope_high < -flat_threshold and slope_low < 0 and slope_high < slope_low:
        pattern_found.append("Falling Wedge (Bullish)")
...
def worker_scan(coin_data):
    try:
        ...
        # Tambah Indikator termasuk NATR
        df = add_indicators(df)
        
        # Ambil nilai NATR dari candle closed (-2)
        ...
        if res:
            ...
            res['natr'] = natr_value # Simpan nilai NATR
            res['df'] = df
...
# ==========================================
def main():
    print(f"🚀 BBMA + SMC + LOSER + NATR BOT (5M)")
    print(f"Target: Top {TOP_COIN_COUNT} Losers | NATR(14) Volatility Check")
    
    global processed_signals

    while True:
        try:
            print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Mengambil daftar koin LOSER...")
            loser_coins = get_loser_symbols(TOP_COIN_COUNT)
            ...
            print(f"-> Memindai {len(loser_coins)} koin Loser...")
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
                futures = {executor.submit(worker_scan, coin): coin['symbol'] for coin in loser_coins}
                ...
                for future in concurrent.futures.as_completed(futures):
                    ...
                    completed += 1
                    if completed % 20 == 0:
                        sys.stdout.write(f"\rProgress: {completed}/{len(loser_coins)}...")
                        sys.stdout.flush()
