...
def send_telegram_alert(symbol, data, image_path):
    ...
    caption = (
        f"🐋 <b>VOLUME + DIVERGENCE ALERT 1H</b>\n"
        f"💎 <b>{symbol}</b>\n"
        f"──────────────────────\n"
...
def generate_chart(df, symbol, signal_info):
    try:
        ...
        title_text = f"{symbol} [1H] - Vol {signal_info['spike_ratio']:.1f}x - {signal_info['divergence']}"
...
# ==========================================
def main():
    print(f"=== VOLUME + DIVERGENCE HUNTER 1H (v3) ===")
    ...
    while True:
        try:
            print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Scanning 1H (Vol + Div)...")
