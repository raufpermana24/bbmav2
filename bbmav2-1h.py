...
def generate_chart(df, symbol, signal_info):
    try:
        ...
        mpf.plot(plot_df, type='candle', style=style, addplot=adds, 
                 title=f"{symbol} [1H] - Vol Spike {signal_info['spike_ratio']:.1f}x", 
...
def analyze_volume_anomaly(df):
    """
    Mengecek apakah Volume 1H terakhir > 2x Rata-rata Volume 24H
    """
    ...
    # Rata-rata 24 candle sebelumnya (24 Jam x 1 candle/jam = 24)
    # Kita ambil slice dari -26 sampai -2 (Total 24 data volume sebelumnya)
    avg_vol_24h = df['volume'].iloc[-26:-2].mean()
...
def worker_scan(symbol):
    try:
        # 1. Ambil Data 1H (Limit 200)
        df = fetch_ohlcv(symbol)
        if df is None: return None

        # 2. FILTER VOLUME (ANOMALI CHECK)
        # Apakah volume 1H terakhir > 2x rata-rata 24 jam terakhir?
...
# ==========================================
def main():
    print(f"=== VOLUME HUNTER 1H + BBMA BOT ===")
    print(f"Strategi: Volume Spike > {VOL_MULTIPLIER}x (vs Avg 24H) di TF 1H.")
    ...
    while True:
        try:
            print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Scan Volume 1H & BBMA...")
            ...
            for alert in alerts_queue:
                sym = alert['symbol']
                # Cek Memory berdasarkan waktu candle 1h
            ...
            # Scan 1 jam sekali atau lebih cepat jika ingin update berjalan
